...
def save_rota_to_excel(
    ...
    """
    Save the generated rota to an Excel workbook.
...
def validate_rota_output(
    ...
    """
    Check that the backend has produced all required columns.
    """
...
def create_weekly_view(
    ...
    """
    Create a simple employee-by-date rota table.

    Each row represents one employee.
    Each date becomes a separate column.
    """

    rota = rota.copy()

    rota["display_shift"] = rota.apply(
        lambda row: (
            "OFF"
            if row["shift"] == "OFF"
...
def create_daily_staffing_summary(
    ...
    """
    Create daily staffing and scheduled-hours totals.
    """

    summary_data = rota.copy()

    summary_data["is_working"] = (
        summary_data["shift"] != "OFF"
    )
...
def format_output_workbook(
    ...
    """
    Apply basic formatting to every Excel worksheet.
    """
